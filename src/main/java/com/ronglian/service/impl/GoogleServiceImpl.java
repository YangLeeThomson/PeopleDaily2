...
public class GoogleServiceImpl implements GoogleService {
	...
	private boolean judgeDeviceParams(Device device){
		...
		flag = flag || StringUtils.isBlank(device.getIdType());
		if(flag){
			return true;
		}
		flag = flag || StringUtils.isBlank(device.getAppVersion());
		if(flag){
			return true;
		}
		flag = flag || StringUtils.isBlank(device.getOsType());
		if(flag){
			return true;
		}
		flag = flag || StringUtils.isBlank(device.getOsVersion());
		if(flag){
			return true;
		}
		flag = flag || StringUtils.isBlank(device.getSdkVersion());
		if(flag){
			return true;
		}
	...
	private String preparedGoogleURL(String adwordUrl,Device device){
		...
		String link_id = null;
		//固定值： iOS  或者 Android
		if("iOS".equals(device.getOsType())){
			link_id = RongLianConstant.iOS_KEY;
		}
		if("Android".equals(device.getOsType())){
			link_id = RongLianConstant.Android_KEY;
		}
		String app_event_type = RongLianConstant.APP_EVENT_TYPE;
		String rdid = device.getRdid();
		String id_type = device.getIdType();
		int lat = device.getLat();
		String app_version = device.getAppVersion();
		String os_version = device.getOsVersion();
		String sdk_version = device.getSdkVersion();
