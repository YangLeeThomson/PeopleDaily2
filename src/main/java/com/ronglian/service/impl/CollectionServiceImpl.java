...
public class CollectionServiceImpl implements CollectionService {
	...
	public RongLianResult getUserCollection(String userId, String deviceId,Integer pageNo, Integer pageSize) {
		...
		if (StringUtils.isBlank(userId)) {
			...
			list = this.collectionDao.selectCollectionListByDeviceIdSort(deviceId,start, pageSize);
		} else {
			list = this.collectionDao.selectCollectionListByUserIdSort(userId,start, pageSize);
		}
...
//		Map<String, Collection> map = new HashMap<String, Collection>();
		...
		for (Collection collect : list) {
			String newsId = collect.getNewsId();
		...
		List<Map> result = this.changeDataContent(list, map);
		return RongLianResult.ok(result);
	}
	 ...
	 * 输出新闻列表的数据转换
	 */
	public List<Map> changeDataContent(List<Collection> list,Map<String, NewsInfo> map) {
